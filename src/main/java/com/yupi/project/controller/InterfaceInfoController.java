...
public class InterfaceInfoController {
    ...
    @Resource
    private dreaifeApiClient dreaifeClient;
...
// 创建一个User对象(这里先模拟一下，搞个假数据)
        com.yupi.dreaifeapiclientsdk.model.User user = new com.yupi.dreaifeapiclientsdk.model.User();
        // 设置user对象的username属性为"test"
        user.setUsername("test");
        // 通过yuApiClient的getUsernameByPost方法传入user对象，并将返回的username赋值给username变量
        String username = dreaifeClient.getUserNameByPost(user);
        // 如果username为空或空白字符串
...
// 返回结果把对象发出去就好了，因为不确定接口的返回值到底是什么
    public BaseResponse<Object> invokeInterfaceInfo(@RequestBody InterfaceInfoInvokeRequest interfaceInfoInvokeRequest,
        ...
        dreaifeApiClient userClient = new dreaifeApiClient(accessKey,secretKey);
        Gson gson = new Gson();
        // 将用户请求参数转换为com.yupi.yuapiclientsdk.model.User对象
        com.yupi.dreaifeapiclientsdk.model.User user = gson.fromJson(userRequestParams, com.yupi.dreaifeapiclientsdk.model.User.class);
